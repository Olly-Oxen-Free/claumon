...
//
// Dedupe is per (gauge, reset window, level): one alert per window, plus one
// more if the projection later hardens from at-risk to likely. A window that
// rolls over gets a fresh ResetAt, so the next window can alert again.
...
func (n *Notifier) shouldSend(a Alert) bool {
