...
- **Wave progress data source confirmed (brief-reviewer finding):** two other real `.nimble/` installations on this machine (`nimbalyst_worktrees/Agent-visibility-and-Workflow-Enhancement` and `.../project-bar-movement`) confirm `status.json` also carries a top-level `waves` field — a map of wave number (string key, e.g. `"1"`) to an object with a `status` field (`pending`/`running`/`complete`/`blocked`). Use `waves[N].status` as the primary source for the "wave progress" success criterion, not just per-task status rollup — read one of those real files if still present on this machine to confirm the exact shape before coding against it, since it wasn't visible from this PRD's own empty-tasks `status.json`.
